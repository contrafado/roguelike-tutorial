...
def heal(*args, **kwargs):
    ...
    if entity.fighter.hp == entity.fighter.max_hp:
        ...
        results.append({'consumed': True, 'message': Message('Your wounds fester somewhat less...', libtcod.green)})
...
def cast_confuse(*args, **kwargs):
    ...
    if not libtcod.map_is_in_fov(fov_map, target_x, target_y):
        results.append({'consumed': False, 'message': Message('You cannot target that which thou cannot see...', libtcod.yellow)})
        return results
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            results.append({'consumed': True, 'message': Message('The eyes roll back in the head of the {0}. It stumbles, throat rattling...'.format(entity.name), libtcod.light_green)})
